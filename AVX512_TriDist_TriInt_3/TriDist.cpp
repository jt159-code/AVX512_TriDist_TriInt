    ...
    const float *B_x, const float *B_y, const float *B_z)
{
    ...
    // 检测退化情况
    __m512 den_abs = _mm512_and_ps(den, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF)));
    __mmask16 degen_mask = _mm512_cmplt_ps_mask(den_abs, eps);
    ...
    // 约束参数（完整版本，与标量逻辑一致）
    // 首先处理 t 的边界情况
    __mmask16 t_lower = _mm512_cmplt_ps_mask(t, zero);
    __mmask16 t_upper = _mm512_cmpgt_ps_mask(t, one);

    // 当 t < 0 时，t = 0，重新计算 u
    __m512 b_dot_b_safe = _mm512_mask_mov_ps(b_dot_b,
                                             _mm512_cmplt_ps_mask(_mm512_and_ps(b_dot_b, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF))), eps),
    ...
    // 然后处理 u 的边界情况
    __mmask16 u_lower = _mm512_cmplt_ps_mask(u, zero);
    __mmask16 u_upper = _mm512_cmpgt_ps_mask(u, one);

    // 当 u < 0 时，u = 0，重新计算 t
    __m512 a_dot_a_safe = _mm512_mask_mov_ps(a_dot_a,
                                             _mm512_cmplt_ps_mask(_mm512_and_ps(a_dot_a, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF))), eps),
                                             _mm512_set1_ps(1.0f)); // 避免除以零
    ...
    int s_edge, int t_edge)
{
    ...
    __mmask16 disjoint_mask = _mm512_cmpgt_ps_mask(cond_val, _mm512_setzero_ps());
    ...
    int shown_disjoint[16])
{
    ...
    for (int edge = 0; edge < 9; edge++)
    {
        ...
        __mmask16 mask = _mm512_cmplt_ps_mask(edge_dist, best_dist);
    ...
    const int shown_disjoint[16])
{
    ...
    // ========== S面投影检查 ==========
    {
        ...
        __mmask16 all_pos = _mm512_kand(
            _mm512_cmpgt_ps_mask(tp0, zero),
            _mm512_kand(
                _mm512_cmpgt_ps_mask(tp1, zero),
                _mm512_cmpgt_ps_mask(tp2, zero)));

        __mmask16 all_neg = _mm512_kand(
            _mm512_cmplt_ps_mask(tp0, zero),
            _mm512_kand(
                _mm512_cmplt_ps_mask(tp1, zero),
                _mm512_cmplt_ps_mask(tp2, zero)));

        __mmask16 valid_mask = _mm512_kor(all_pos, all_neg);

        // 排除退化和跳过情况
        __mmask16 degen_mask = _mm512_cmplt_ps_mask(snl2, eps);
        __mmask16 final_mask = _mm512_kand(valid_mask,
                                           _mm512_kandnot(degen_mask, skip_mask));

        if (final_mask != 0)
        {
            // 找到最小绝对值
            __m512 tp0_abs = _mm512_and_ps(tp0, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF)));
            __m512 tp1_abs = _mm512_and_ps(tp1, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF)));
            __m512 tp2_abs = _mm512_and_ps(tp2, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF)));

            __mmask16 mask01 = _mm512_cmplt_ps_mask(tp1_abs, tp0_abs);
            __m512 min_abs = _mm512_mask_mov_ps(tp0_abs, mask01, tp1_abs);
            __m512 min_val = _mm512_mask_mov_ps(tp0, mask01, tp1);

            __mmask16 mask02 = _mm512_cmplt_ps_mask(tp2_abs, min_abs);
            ...
            __mmask16 update_mask = _mm512_kand(final_mask,
                                                _mm512_cmplt_ps_mask(new_dist, cur_dist));
    ...
    // ========== T面投影检查（对称处理）=========
    {
        ...
        // 检查同号
        __mmask16 all_pos = _mm512_kand(
            _mm512_cmpgt_ps_mask(sp0, zero),
            _mm512_kand(
                _mm512_cmpgt_ps_mask(sp1, zero),
                _mm512_cmpgt_ps_mask(sp2, zero)));

        __mmask16 all_neg = _mm512_kand(
            _mm512_cmplt_ps_mask(sp0, zero),
            _mm512_kand(
                _mm512_cmplt_ps_mask(sp1, zero),
                _mm512_cmplt_ps_mask(sp2, zero)));

        __mmask16 valid_mask = _mm512_kor(all_pos, all_neg);

        __mmask16 degen_mask = _mm512_cmplt_ps_mask(tnl2, eps);
        __mmask16 final_mask = _mm512_kand(valid_mask,
                                           _mm512_kandnot(degen_mask, skip_mask));
        ...
        {
            // 找到最小绝对值
            __m512 sp0_abs = _mm512_and_ps(sp0, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF)));
            __m512 sp1_abs = _mm512_and_ps(sp1, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF)));
            __m512 sp2_abs = _mm512_and_ps(sp2, _mm512_castsi512_ps(_mm512_set1_epi32(0x7FFFFFFF)));

            __mmask16 mask01 = _mm512_cmplt_ps_mask(sp1_abs, sp0_abs);
            __m512 min_abs = _mm512_mask_mov_ps(sp0_abs, mask01, sp1_abs);
            __m512 min_val = _mm512_mask_mov_ps(sp0, mask01, sp1);

            __mmask16 mask02 = _mm512_cmplt_ps_mask(sp2_abs, min_abs);
