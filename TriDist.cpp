...
struct alignas(64) TriangleBatch16
{
    ...
    float P_x[16], P_y[16], P_z[16]; // 最近点对（S上的点）
...
struct alignas(64) EdgePairResults16
{
    ...
    float vec_x[9][16], vec_y[9][16], vec_z[9][16]; // 向量差
    // 计算线段最近点的参数
    ...
    int s_edge, int t_edge)
{
    ...
    // 两个三角形可能因为互相穿插而相交，也可能因为平行且投影重叠而相交，还可能因为边或顶点刚好接触而相交。
    ...
    {
        if (mask_int & (1 << i))
            // 跳过已经证明分离的三角形对（不再进行顶点投影测试）
            results.shown_disjoint[i] = 1;
    }
}

// ========== 选择最优最近点（添加预取） ==========
// 预取真的加速了吗？按理大规模 数据下应该会加速吧？如果数据量较小，预取可能反而增加开销。需要实际测试验证。
    ...
    const int shown_disjoint[16])
{
    ...
    // 创建skip mask
    // _mm512_set_epi32 是一个内置函数，它的语义是将参数按照从最高索引到最低索引的顺序放置到向量寄存器中
    // 使用__m512i skip = _mm512_load_si512(shown_disjoint);（或loadu）可以吗？
    ...
    // ========== S面投影检查 ==========
    {
        ...
        if (final_mask != 0)
        {
            ...
            // 计算距离（可以new_dist = |t_val| * sqrt(snl2)，但在三角形距离计算中，仅仅输出距离有时是不够的。许多应用场景需要知道具体哪两个点最近）
            // 三个顶点到 S 平面的有符号距离同号，且我们选取的是绝对值最小的那个顶点。根据三角形凸性，该顶点的投影点必定落在 S 三角形内部（或边界上）。这是经典三角形最近点算法的结论。所以代码中不需要额外判断投影点是否在三角形内，直接使用即可。
            ...
            // 只有满足最终条件（同号、非退化、未分离）且新距离比当前距离小的三角形对，才需要更新
    ...
    const PQP_REAL t_batch[16][3][3])
{
    ...
    // 填充batch，并在循环中预取下一组输入数据（AoS → SoA 转换）
    for (int i = 0; i < 16; i++)
    {
        // 拷贝当前三角形的顶点
