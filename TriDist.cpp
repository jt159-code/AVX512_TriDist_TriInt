...
// ========== 批量三角形距离计算主函数 (16对) ==========
/**
 * 批量计算16对三角形之间的最近点和距离（AVX4优化版本）
 *
 * 使用SIMD指令并行处理16对三角形，计算每对三角形的最近点对及其欧氏距离。
 * 算法采用边-边最近点计算和顶点投影检查相结合的方式，确保找到全局最优解。
 *
 * @param p_batch [16][3] 输出参数，存储S三角形上的最近点坐标
 * @param q_batch [16][3] 输出参数，存储T三角形上的最近点坐标
 * @param dist [16] 输出参数，存储每对三角形的欧氏距离
 * @param s_batch [16][3][3] 输入参数，S三角形的顶点坐标（16个三角形，每个3个顶点，每个顶点3个坐标）
 * @param t_batch [16][3][3] 输入参数，T三角形的顶点坐标（16个三角形，每个3个顶点，每个顶点3个坐标）
 */
void TriDistBatch16(
